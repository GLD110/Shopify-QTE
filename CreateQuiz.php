...
if(isset($_REQUEST['quizname'])) {
	...
	//$post = $shopify("POST /admin/pages.json",$post_page);
	/*create page*/
	if($_REQUEST['hidden-quiz-id'] == '') {
		$db->query("INSERT INTO `tbl_quiz_details` (`quizname`, `quizstyle`, `slidetime`, `timedpoptime`, `buttoncolor`, `fontcolor`, `question`, `questionstyle`, `answer`, `answerimage`, `excludepage`, `mainpagetext`, `opttextbefore`, `opttextafter`, `pbar`,`store_name`,`status`,`backgroundcolor`,`backgroundimage`,`color`,`mainpageimage`,`description`,`mainbuttontext`,`optcode`) VALUES ('".$_REQUEST['quizname']."', '".$_REQUEST['quizstyle']."', '".$_REQUEST['slidetime']."', '".$_REQUEST['timedpoptime']."', '".$_REQUEST['buttoncolor']."', '".$_REQUEST['fontcolor']."', '".json_encode(array_values($_REQUEST['question']))."', '".json_encode(array_values($_REQUEST['questionstyle']))."', '".json_encode(array_values($_REQUEST['answer']))."', '".json_encode($AnswerImage)."', '".json_encode($_REQUEST['excludepage'])."', '".$_REQUEST['mainpagetext']."','".$_REQUEST['opttextbefore']."', '".$_REQUEST['opttextafter']."', '".$_REQUEST['pbar']."','".$_SESSION['shop']."',1,'".$_REQUEST['backgroundcolor']."','".$backImage."','".$_REQUEST['color']."','".$mainImage."','".$_REQUEST['description']."','".$_REQUEST['mainbuttontext']."','".$optcode."')");
		$_SESSION['alert'] = 'Succesfully Inserted';
		$_SESSION['last_id'] =$db->insert_id;
	} else {
		$db->query("UPDATE tbl_quiz_details SET quizname='".$_REQUEST['quizname']."',quizstyle='".$_REQUEST['quizstyle']."',slidetime='".$_REQUEST['slidetime']."',timedpoptime='".$_REQUEST['timedpoptime']."',buttoncolor='".$_REQUEST['buttoncolor']."',fontcolor='".$_REQUEST['fontcolor']."',question='".json_encode(array_values($_REQUEST['question']))."',questionstyle='".json_encode(array_values($_REQUEST['questionstyle']))."',answer='".json_encode($answer)."',answerimage='".json_encode($AnswerImage)."',excludepage='".json_encode($_REQUEST['excludepage'])."',mainpagetext='".$_REQUEST['mainpagetext']."', opttextbefore='".$_REQUEST['opttextbefore']."',opttextafter='".$_REQUEST['opttextafter']."',pbar='".$_REQUEST['pbar']."', backgroundimage='".$backImage."',backgroundcolor='".$_REQUEST['backgroundcolor']."',color='".$_REQUEST['color']."', mainpageimage='".$mainImage."', description='".$_REQUEST['description']."', mainbuttontext='".$_REQUEST['mainbuttontext']."', optcode='".$optcode."' WHERE id='".$_SESSION['last_id']."'");
				...
				<form class="form-horizontal" method="post" enctype="multipart/form-data" id="QuizForm">
					...
					<div class="form-group">
						...
						<div class="col-sm-8">
							...
							<input type="hidden" name="hidden-quiz-id" value="<?php echo $_SESSION['last_id'];?>">
						</div>
					</div>
					<!-- end Quiz Name -->
					<!-- Quiz Styles -->
					<div class="form-group">
						...
						<div class="col-sm-8 quizstyle">
							<div class="col-md-3">
								<label>
									<?php if($QuizDetail['quizstyle'] == 'corner' || $QuizDetail['quizstyle'] =='') { $slideup = 'checked'; $disabled = ''; } else { $disabled = 'disabled'; } ?>
					...
					<div class="form-group">
						...
						<div class="col-sm-2">
							<div class="input-group my-colorpicker2">
								<?php if($QuizDetail['buttoncolor'] == '') { ?>
								<input type="text" class="form-control" name="buttoncolor" value="#f81c1c">
								<?php } else { ?>
					...
					<div class="form-group">
						...
						<div class="col-sm-2">
							<div class="input-group my-colorpicker2">
								<?php if($QuizDetail['fontcolor'] == '') { ?>
								<input type="text" class="form-control" name="fontcolor" value="#220909">
								<?php } else { ?>
						...
						<div class="col-sm-6 col-sm-offset-1">
							<div class="col-sm-4">
								<button class="btn btn-default backgroundimage">Background Color</button>
									...
									<?php if($QuizDetail['backgroundcolor'] == '') { ?>
									<input type="text" class="form-control" name="backgroundcolor" id="backgroundcolor" value="#ffffff">
									<?php } else { ?>
					...
					<div class="form-group">
						...
						<div class="col-sm-8">
							<label class="checkbox-inline">
								<input type="checkbox" name="pbar" <?php if($QuizDetail['pbar'] == 1 || $QuizDetail['pbar'] == '') { echo "checked";}?> data-toggle="toggle" value="<?php echo $QuizDetail['pbar'];?>">
							</label>
						</div>
					</div>
					<!-- End Progress bar-->
									...
									$excludepage = json_decode($QuizDetail['excludepage'],true);
										foreach ($excludepage as $value) {
 										...
 										if(strpos($page['title'],'Home') !==false) { ?>
											<option value="home" <?php echo $page_select;?>><?php echo $page['title'];?></option>
 										<?php } else { ?>
					...
					<div class="form-group">
						...
						<div class="col-sm-8 col-sm-offset-2">
							<?php if($QuizDetail['description'] == '') { ?>
							<textarea rows="5" cols="100" class="form-control" name="description" required></textarea>
							<?php } else { ?>
					...
					<div class="form-group">
						...
						<div class="col-sm-8 col-sm-offset-2">
							<?php if($QuizDetail['mainbuttontext'] == '') { ?>
							<input type="text" class="form-control text-center" name="mainbuttontext" value="Take The Quiz!">
							<?php } else { ?>
					...
					<div class="form-group">
						...
						<div class="col-sm-8 col-sm-offset-2">
							<?php if($QuizDetail['opttextbefore'] == '') { ?>
							<input type="text" class="form-control text-center" name="opttextbefore" value="Thank you for taking the quiz enter your email to save $15">
							<?php } else { ?>
					...
					<div class="form-group">
						...
						<div class="col-sm-8 col-sm-offset-2">
							<?php if($QuizDetail['opttextafter'] == '') { ?>
							<textarea class="form-control text-center" name="opttextafter">Thank you for entering your email.Your coupon code is save15. Enter it at checkout to save $15 on your order.</textarea>
							<?php } else { ?>
